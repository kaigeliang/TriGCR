...
def prediction(data, processed_list, input_builder, model):
    ...
    prompt_build_time = elapsed_since(input_start)
    if trie is None:
        return None
    ...
    prompt_build_time += elapsed_since(token_start)
    prediction, kg_decode_time = timed_call(
    ...
    n_paths = len(prediction) if isinstance(prediction, list) else 1
    result = {
        "id": id,
        "question": question,
        ...
        "timing": {
            "schema_version": 1,
            "unit": "seconds",
            "method": "baseline_path_generation",
            "total": total_time,
            "path_generation_total": total_time,
            "prompt_build_time": prompt_build_time,
            "kg_decode_time": kg_decode_time,
            "num_kg_decode_calls": 1,
            "num_api_calls": 0,
            "num_paths": n_paths,
            "model_name": getattr(getattr(model, "args", None), "model_name", None),
        },
    }
    return result
...
def main(args, LLM):
    ...
    write_timing_summary(os.path.join(output_dir, 'predictions.jsonl'))
