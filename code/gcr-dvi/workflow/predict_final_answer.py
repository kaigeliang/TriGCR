...
def make_prediction(data, args, processed_list, input_builder, model):
    ...
    if id in processed_list:
        return None
    path_timing = _load_timing(data.get("path_timing"))
    path_total = _as_float(path_timing.get("total"))
    if path_total is None:
        path_total = _as_float(path_timing.get("path_generation_total"))

    if model is None:
        prediction, final_answer_time = timed_call(input_builder.direct_answer, data)
        final_total = elapsed_since(sample_start)
        total_time = final_total + path_total if path_total is not None else final_total
        return {
            "id": id,
            "question": question,
            ...
            "timing": {
                "schema_version": 1,
                "unit": "seconds",
                "method": "baseline_final_answer",
                "total": total_time,
                "final_answer_total": final_total,
                "final_answer_generation_time": final_answer_time,
                "path_generation_total": path_total,
                "path_generation": path_timing,
                "num_api_calls": 0,
                "model_name": None,
            },
        }
    ...
    prompt_build_time = elapsed_since(prompt_start)
    ...
    if "gcr" in args.model_name:
        ...
        prompt_build_time += elapsed_since(generation_start)
        prediction, final_answer_time = timed_call(
        ...
        is_api_model = args.model_name.startswith("gpt")
        prediction, final_answer_time = timed_call(
            model.generate_sentence,
            input,
            sync_cuda=not is_api_model,
        )
    if prediction is None:
        return None
    final_total = elapsed_since(sample_start, sync_cuda=not args.model_name.startswith("gpt"))
    total_time = final_total + path_total if path_total is not None else final_total
    is_api_model = args.model_name.startswith("gpt")
    result = {
        "id": id,
        ...
        "timing": {
            ...
            "final_answer_generation_time": final_answer_time,
            "final_api_time": final_answer_time if is_api_model else 0.0,
            "final_local_llm_time": 0.0 if is_api_model else final_answer_time,
            "path_generation_total": path_total,
            "path_generation": path_timing,
            "num_api_calls": 1 if is_api_model else 0,
...
def merge_path_result(
    ...
    def find_path(sample):
        ...
        if qid in question_to_path:
            ...
            sample["path_timing"] = json.dumps(
                question_to_path[qid].get("path_timing", {}),
                ensure_ascii=False,
            )
        else:
...
def main(args, LLM):
    ...
    write_timing_summary(os.path.join(output_dir, f"predictions.jsonl"))
