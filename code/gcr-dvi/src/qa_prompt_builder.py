...
class DVIPromptBuilder(JointReasoningPromptBuilder):
    """
    DVI (Decompose-Verify-Intersect) 专用 PromptBuilder.

    核心新增方法:
      get_per_constraint_tries(question_dict, constraints)
        → Dict[constraint_id → MarisaTrie]

    每个约束只从其 anchor 实体出发做 DFS, 构建独立的 mini-Trie.
    比全局 Trie 更小、更精准, 解码结果直接对应单个约束的候选集.
    """
...
"""
    ...
    CONSTRAINT_PROMPT = """Reasoning path is a sequence of triples in the KG that connects the topic entities in the question to answer entities. Given a question, please generate some reasoning paths in the KG starting from the topic entities to answer the question.
...
"""
    ...
    def get_per_constraint_tries(
        self,
        question_dict: dict,
        constraints: list,
    ) -> dict:
        """
        为每个 relation/attribute 约束分别构建 mini-Trie.

        参数:
            question_dict: 数据集中的一条样本
            constraints:   decomposer 返回的约束列表
                           (只处理 type in ['relation', 'attribute'] 的条目)

        返回:
            {constraint_id: MarisaTrie}
            若某个 anchor 在图中找不到路径, 该 id 不会出现在结果中.
        """
        g = utils.build_graph(question_dict["graph"], self.undirected)
        tries = {}
        self.last_trie_stats = {}

        for c in constraints:
            if c.get("type") not in ("relation", "attribute"):
                continue
            ...
            # 只从单个 anchor 出发做 DFS, 构建更小的 Trie
            paths = utils.dfs(g, [anchor], self.index_path_length)
            self.last_trie_stats[cid] = {
                "anchor": anchor,
                "n_paths": len(paths),
            }
            ...
            path_strs = [
                f"{self.PATH_START_TOKEN}{utils.path_to_string(p)}{self.PATH_END_TOKEN}"
                for p in paths
            ]
            tokenized = self.tokenizer(
                path_strs, padding=False, add_special_tokens=False
            ).input_ids
            tries[cid] = MarisaTrie(
                tokenized, max_token_id=len(self.tokenizer) + 1
            )
    ...
    def build_constraint_prompt(
        ...
        Keep the prompt shape identical to the original GCR path-generation
        prompt. The KG-specialized model was tuned on # Question / # Topic
        entities; introducing # Anchor entity / # Condition to verify shifts it
        out of distribution and can make it echo the condition as an answer.
        """
        return self.CONSTRAINT_PROMPT.format(
            question=question,
            entities=anchor,
        )

    def get_per_constraint_paths(
        self,
        question_dict: dict,
        constraints: list,
    ) -> dict:
        """
        Enumerate DFS paths per relation/attribute constraint.

        This is the PathScorer equivalent of get_per_constraint_tries(): it
        returns plain path strings and does not require a tokenizer.
        """
        g = utils.build_graph(question_dict["graph"], self.undirected)
        ...
        for c in constraints:
            ...
            paths_per_constraint[cid] = [utils.path_to_string(p) for p in paths]

        return paths_per_constraint

    def get_fallback_paths(self, question_dict: dict) -> list:
        """Enumerate global DFS paths from all question entities."""
        if "paths" in question_dict:
            paths = question_dict["paths"]
        else:
            g = utils.build_graph(question_dict["graph"], self.undirected)
            paths = utils.dfs(
                g,
                question_dict["q_entity"],
                self.index_path_length,
            )
        return [utils.path_to_string(p) for p in paths]

    def build_fallback_trie(self, question_dict: dict):
        """
        Fallback: 若 decompose 失败, 退回标准 GCR 全局 Trie.
        与 baseline PathGenerationWithAnswerPromptBuilder 行为一致.
        """
        return self.get_graph_index(question_dict)
