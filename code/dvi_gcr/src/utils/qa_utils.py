...
try:
    from .timing_utils import write_timing_summary
except ImportError:
    from src.utils.timing_utils import write_timing_summary


def _record_inference_time(record):
    value = record.get("inference_time")
    if isinstance(value, (int, float)) and not isinstance(value, bool):
        return float(value)
    timing = record.get("timing")
    if isinstance(timing, dict):
        for key in ("inference_time", "total"):
...
def _append_timing_metrics(result_str: str, predict_file: str) -> str:
    """Always expose inference-time coverage in eval_result.txt."""
    try:
        summary = write_timing_summary(predict_file)
    except Exception:
        return (
            result_str
            + " InferenceTimeMeanSec: N/A"
            + " InferenceTimeTotalSec: N/A"
            + " TimingCoverage: 0/0"
        )

    metrics = summary.get("metrics", {})
    timing_metric = metrics.get("inference_time") or metrics.get("total")
    coverage = f"{summary.get('n_with_timing', 0)}/{summary.get('n_records', 0)}"
    if not timing_metric:
        return (
            result_str
            + " InferenceTimeMeanSec: N/A"
            + " InferenceTimeTotalSec: N/A"
            + f" TimingCoverage: {coverage}"
        )

    return (
        result_str
        + f" InferenceTimeMeanSec: {timing_metric['mean']}"
        + f" InferenceTimeTotalSec: {timing_metric['sum']}"
        + f" TimingCoverage: {coverage}"
    )
...
def eval_hit_at1(prediction, answer):
    if isinstance(prediction, list):
        first = ""
        for item in prediction:
            if str(item).strip():
                first = str(item).strip()
                break
    else:
        first = str(prediction).split("\n")[0].strip()
    if not first:
        return 0
    return eval_hit(first, answer)
...
def eval_rank_results(predict_file, topk=[1, 3, 5, 10]):
    ...
    result_str = _append_timing_metrics(result_str, predict_file)
    print(result_str)
...
def eval_result(predict_file, cal_f1=True, topk=-1):
    ...
    if len(f1_list) > 0:
        result_str = (
            ...
            + " Hit@1: "
            + str(sum(hit1_list) * 100 / len(hit1_list))
            + " F1: "
