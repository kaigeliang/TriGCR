...
\usepackage[letterpaper,margin=0.62in,columnsep=0.18in]{geometry}
...
\title{TriGCR: Constraint-Aware KG-Trie Reasoning for Knowledge-Grounded LLMs:\
...
The project requirements ask us to reproduce the baseline, explore more than two KG-specialized LLMs, analyze failures by query type, and modify the graph-constrained decoding module for complex reasoning constraints. This expanded technical report is organized around those requirements but is no longer page-limited: it includes the reasoning motivation, detailed algorithms, implementation decisions, experiment protocols, ablations, failure analysis, and reproduction notes.
...
\begin{figure*}[t]
\centering
\includegraphics[width=\textwidth]{trigcr_project_overview.png}
...
\section{Research Questions and Design Principles}
...
\subsubsection{DVI Algorithm}
Figure~\ref{fig:dvi-algorithm} gives the implemented DVI control flow. The important distinction from baseline GCR is that DVI does not ask the final LLM to discover the intersection implicitly. It materializes candidate sets and records the intersection statistics before final answer generation.
...
\small
\textbf{Algorithm 1: DVI inference for one question.}
\begin{enumerate}
  \item Input question $q$, topic entities $E_q$, local graph $G_q$, KG model $M_{KG}$, general model $M_G$.
  \item Decompose $q$ into a JSON object with answer variable, relation/attribute constraints, and filter constraints. If decomposition is unavailable or not useful, create a fallback global constraint.
  \item For each relation-like constraint $c_i$, collect DFS paths from the relevant anchor entity. Either build a mini-trie and decode with $M_{KG}$, or rank enumerated paths with PathScorer.
  \item Extract tail entities from accepted paths to form $S_i$. Record path counts, trie statistics, and whether selective fallback was used.
  \item Compute $C_{strict}=\bigcap_i S_i$. If $C_{strict}$ is empty and \code{min\_candidates} is positive, compute relaxed candidates by dropping constraints in a controlled diagnostic order.
  \item Optionally apply answer-aware refinement to expand mediator endpoints toward typed final entities.
  \item Build the final answer prompt from evidence paths plus candidate/intermediate entities. Generate concise answer strings with $M_G$.
  \item Save answer, paths, final candidates, raw candidates, decomposition metadata, intersection metadata, and timing metadata.
\end{enumerate}
\end{minipage}}
\caption{DVI controller algorithm. The controller exposes intermediate state that is hidden in the baseline GCR path-to-answer pipeline.}
\label{fig:dvi-algorithm}
\end{figure*}

\subsubsection{Decomposition Details}
The decomposer uses a general LLM with deterministic temperature to produce JSON. The schema contains an \code{answer\_variable} and a list of constraints. Relation and attribute constraints require an anchor from the topic entities, while filter constraints contain a predicate-like description. The implementation includes a retry loop, cache support through \code{decompose\_cache\_path}, validation of required fields, and a single-entity fallback that avoids unnecessary API calls.
...
\subsubsection{Candidate Extraction and Intersection}
...
\subsubsection{Answer-Aware Refinement}
...
\subsubsection{GraphLite Algorithmic Motivation}
GraphLite changes the unit of search. Baseline GCR searches in token space under a trie. GraphLite searches in graph space by enumerating relations and entities, then scores structured candidates. This makes the verifier more inspectable: relation selection errors, entity scoring errors, and expansion errors can be analyzed separately.
...
\subsubsection{Integrated PathScorer Variant}
The integrated code path is more lightweight than the full Lite prototype. Instead of training a complete entity-level decoder, \code{PathScorer} first uses \code{sentence-transformers/all-MiniLM-L6-v2} as a bi-encoder to filter a large path pool to \code{bi\_k} candidates, then uses \code{cross-encoder/ms-marco-MiniLM-L-6-v2} to rerank to \code{cross\_k}. The query is built from the full question plus the constraint hint when available. This design trades completeness for speed: it avoids KG-LLM decoding, but it depends on path text matching the question semantics.

\subsubsection{Why GraphLite Underperforms End-to-End}
GraphLite's lower F1 is not simply a failed implementation. It reveals a real modeling issue. A cross-encoder can identify paths that are semantically similar to the question, but KGQA often requires factual knowledge, answer typing, and multi-constraint aggregation. A path that mentions a country, anthem, or office may be semantically related but still answer the wrong predicate. This is why the path-reranker training improves Hit@1 and MRR at the path level while the final QA score remains below strong GCR baselines. The best future role for GraphLite is therefore a verifier, prefilter, or feature generator inside a larger DVI-style controller.
...
\begin{figure*}[t]
\centering
\includegraphics[width=\textwidth,height=0.48\textheight,keepaspectratio]{trigcr_method_architecture.png}
...
\subsubsection{Embedding-Guided Path Selection Algorithm}
...
The key engineering separation is between \emph{collection budget} and \emph{trie budget}. A larger collection budget allows the system to look beyond short local paths, especially at 3 hops. A smaller trie budget prevents the decoder from being overwhelmed by irrelevant branches. This separation is what makes preserve-2 plus emb-3 meaningful: it keeps the reliable 2-hop evidence while allowing selected 3-hop evidence to enter.

\subsubsection{Why Naive 3-Hop Expansion Fails}
...
\begin{figure*}[t]
\centering
\includegraphics[width=\textwidth,height=0.38\textheight,keepaspectratio]{trigcr_reasoning_traces.png}
...
\subsection{Prediction Metadata and Saved State}
...
\bottomrule
\end{tabular}
\caption{DVI metadata groups. Saving this state is essential for interpreting why a prediction succeeds or fails.}
\label{tab:metadata-schema}
\end{table*}

\subsection{Code Organization}
The GitHub artifact keeps the three method tracks separated. The main implementation is in \path{code/dvi_gcr}; the embedding-guided KG-Trie patch is in \path{code/embedding_guided_kgtrie}; and the standalone GraphLite prototype is in \path{code/graphlite}. The separation is deliberate: DVI, GraphLite, and embedding-guided path selection answer different engineering questions and should be testable independently before being merged into one pipeline.

\subsection{Caching, Runtime, and Reproducibility}
...
A method can improve Hit while hurting Precision if it returns more candidates. It can improve Hit@1 while hurting F1 if it chooses one strong answer but omits additional gold answers. It can also improve F1 while being too slow for practical use. This is why the report does not reduce the comparison to a single number. DVI is judged by full-test F1 and Hit@1, GraphLite by speed-quality tradeoff, and embedding-guided KG-Trie by whether additional hops improve controlled probes without sacrificing full-test behavior.
...
\subsection{Detailed Interpretation of the Full-Test Table}
...
\subsection{Statistical Caution}
...
\subsection{Embedding Experiments as Negative and Positive Evidence}
...
\subsection{Implications for a Unified System}
...
\subsection{Failure Analysis Methodology}
...
\subsection{What the Ablations Show Together}
...
\section{Detailed Design Lessons}
\paragraph{Lesson 1: Faithfulness and correctness are different.}
...
\paragraph{Lesson 2: Intersections should be programmatic.}
...
\paragraph{Lesson 3: Longer paths need admission control.}
Increasing hop length is tempting because it improves coverage, but the path space expands quickly and generic entities dominate. Embedding-guided selection is useful when it acts as admission control for additional evidence, not when it replaces all short-hop graph coverage.

\paragraph{Lesson 4: Fast verifiers need answer semantics.}
...
\paragraph{Lesson 5: Routing is a modeling problem.}
The oracle routing table shows large headroom, but exact-size routing captures only a small part of it. A learned router should be trained on validation data and should use features from all modules: baseline answer confidence, DVI candidate-set size, decomposition validity, path-score margins, answer-type confidence, and timing.
...
\subsection{Expanded Future Work}
...
\begin{itemize}
  \item \textbf{kaigeliang}: DVI track, including question decomposition, per-constraint verification, candidate intersection, answer-aware refinement, routed DVI experiments, failure analysis, and final report integration.
  \item \textbf{fangzhehuang}: GraphLite track, including entity-level graph enumeration, relation/entity verifier design, PathScorer integration, runtime comparison, and GraphLite experiment summaries.
  \item \textbf{yaoxianshan}: embedding-guided KG-Trie track, including semantic path ranking, 3-hop path-budget experiments, embedding ablations, artifact cleanup, and poster preparation.
\end{itemize}
...
The released code does not bundle the full RoG data or Freebase-derived graph files because they are large benchmark artifacts. The submission package instead includes \path{datasets/DATASET_LINKS.md}. The reproduction commands use the Hugging Face dataset identifiers \code{rmanluo/RoG-cwq} and \code{rmanluo/RoG-webqsp}, corresponding to \href{https://huggingface.co/datasets/rmanluo/RoG-cwq}{RoG-CWQ} and \href{https://huggingface.co/datasets/rmanluo/RoG-webqsp}{RoG-WebQSP}. Local experiments in the archive load these datasets through the same \code{--data\_path rmanluo --d RoG-cwq} and \code{--data\_path rmanluo --d RoG-webqsp} conventions.

\subsection{Prompt Templates}
This project uses prompts in two places: the KG-specialized model generates graph-valid reasoning paths under a trie, and the general LLM performs decomposition and final answer synthesis. GraphLite is intentionally different: it replaces the KG-LLM prompt with structured graph enumeration and learned scoring inputs. The templates below are the report-facing versions of the code prompts; placeholders such as \code{\{question\}} and \code{\{entities\}} are filled by the workflow scripts.

\paragraph{Baseline GCR and embedding-guided KG-Trie path prompt.}
This prompt is used by the baseline path-generation workflow and by the embedding-guided overlay after the path pool has been ranked and admitted into the KG-Trie.
\begin{verbatim}
...
some reasoning paths in the KG starting from the topic entities to answer the
question.
...
\paragraph{DVI decomposition system prompt.}
The decomposition call asks a general LLM to produce strict JSON rather than prose. This keeps the controller deterministic after the LLM call.
\begin{verbatim}
You are a knowledge graph query analyzer.
Decompose a complex question into atomic constraints, each anchored to ONE known
entity. Output valid JSON only. No extra text.
\end{verbatim}

\paragraph{DVI decomposition user prompt.}
The user prompt defines the output schema and gives few-shot examples. The key design decision is that relation and attribute constraints must be anchored to known question entities, while literal constraints become filter predicates.
\begin{verbatim}
Decompose the question into atomic constraints over the answer variable.
...
- "filter":     the answer must satisfy a literal predicate (date, type, count,
                nationality)

Rules:
1. Every "relation" or "attribute" constraint must have an "anchor" from q_entities.
2. Every "filter" constraint must have a "predicate" (Python-evaluable string when
   possible).
...
Example output:
{
  ...
  "constraints": [
    {"id": "c1", "type": "relation",  "anchor": "Christopher Nolan",
     "hint": "films directed by Christopher Nolan starring the actor"},
...
\paragraph{DVI per-constraint mini-trie prompt.}
After decomposition, DVI builds a separate trie from each constraint anchor. The text prompt remains close to GCR, but the graph constraint and topic-entity list are now scoped to one atomic condition.
\begin{verbatim}
Reasoning path is a sequence of triples in the KG that connects the topic
entities in the question to answer entities. Given a question, please generate
some reasoning paths in the KG starting from the topic entities to answer the
...
\paragraph{Final answer prompt with candidates.}
DVI deliberately tells the final LLM that candidate entities may be intermediate. This was added after failure analysis showed that strict endpoint selection can return mediator nodes or broad type entities.
\begin{verbatim}
Based on the reasoning paths below, answer the original question.
The candidate entities are KG entities collected from path endpoints; they may be
intermediate or noisy, so do not restrict your answer to the candidate list.
Return all possible final answers, one per line. Keep answers concise (entity
names only).
...
\paragraph{Final answer prompt without candidates.}
This fallback is used when no reliable final candidate set is available but evidence paths still exist.
\begin{verbatim}
...
\paragraph{GraphLite scoring inputs.}
GraphLite does not use a natural-language generation prompt during verification. Instead it converts verification into supervised scoring examples. Relation selection scores \code{(question, relation)} pairs, and entity verification scores \code{(question, path, entity)} triples. This is the main architectural difference between GraphLite and KG-LLM DVI: the output space is a ranked list of graph objects rather than generated path text.
...
PYTHONPATH=code/dvi_gcr python code/dvi_gcr/workflow/predict_paths_and_answers.py \
...
\subsection{Prediction Record Schema}
...
\begin{itemize}
  \item \code{id}, \code{question}, \code{prediction}, and optional gold-answer fields.
  \item \code{decomposition}: answer variable, relation constraints, attribute constraints, and filters.
  \item \code{paths\_per\_constraint}: accepted or ranked evidence paths for each constraint.
  \item \code{raw\_final\_candidates} and \code{final\_candidates}: candidates before and after answer-aware refinement.
  \item \code{intersect\_stats}: strict size, relaxed size, dropped constraints, and candidate-set diagnostics.
  \item \code{timing}: decomposition, KG decoding, path scoring, intersection, refinement, final answer, and total time.
\end{itemize}

\subsection{Detailed Artifact Map}
\begin{table*}[h]
\centering
\small
...
\midrule
\path{code/dvi_gcr/workflow/predict_dvi.py} & End-to-end DVI pipeline with KG-LLM and PathScorer verification modes. \\
\path{code/dvi_gcr/src/dvi/decomposer.py} & General-LLM JSON decomposer with fallback and cache support. \\
\path{code/dvi_gcr/src/dvi/intersector.py} & Tail-entity extraction, candidate-set construction, strict intersection, and relaxed diagnostics. \\
\path{code/dvi_gcr/src/dvi/answer_aware.py} & Heuristic answer-type refinement and local graph expansion. \\
\path{code/dvi_gcr/src/dvi/path_scorer.py} & Bi-encoder filtering and cross-encoder reranking for enumerated paths. \\
\path{code/embedding_guided_kgtrie/overlay/src/qa_prompt_builder.py} & Embedding-guided path ranking and KG-Trie construction overlay. \\
\path{code/graphlite/lite_framework/entity_level_decoder.py} & Standalone GraphLite entity-level graph enumeration and scoring prototype. \\
\path{results_summary/EXPERIMENT_TABLES.md} & GitHub-readable experiment tables corresponding to the report. \\
\bottomrule
\end{tabular}
\caption{Expanded artifact map for reproducing and extending the project.}
\label{tab:artifact-map}
\end{table*}

\subsection{Code Locations}
\begin{itemize}
  \item DVI implementation: \path{code/dvi_gcr/workflow/predict_dvi.py}, \path{code/dvi_gcr/src/dvi/decomposer.py}, \path{code/dvi_gcr/src/dvi/intersector.py}, \path{code/dvi_gcr/src/dvi/answer_aware.py}, and \path{code/dvi_gcr/src/qa_prompt_builder.py}.
  \item GraphLite entity-level prototype: \path{code/graphlite/lite_framework.rar}; extracted source files are packaged at \path{code/graphlite/lite_framework/entity_level_decoder.py}, \path{code/graphlite/lite_framework/cross_encoder_verifier.py}, \path{code/graphlite/lite_framework/graph_walker.py}, \path{code/graphlite/lite_framework/graph_encoder.py}, \path{code/graphlite/lite_framework/train_cross_encoder.py}, and \path{code/graphlite/lite_framework/train_entity_verifier.py}.
  \item Integrated GraphLite / PathScorer implementation: \path{code/dvi_gcr/src/dvi/path_scorer.py}, \path{code/dvi_gcr/workflow/build_path_verifier_data.py}, and \path{code/dvi_gcr/workflow/train_path_reranker.py}.
  \item Embedding-guided KG-Trie implementation: \path{code/embedding_guided_kgtrie/overlay/src/qa_prompt_builder.py} and \path{code/embedding_guided_kgtrie/overlay/workflow/predict_paths_and_answers.py}.
  \item Packaged result summaries: \path{results_summary/EXPERIMENT_TABLES.md} and \path{results_summary/RESULTS.md}. These files contain the full-test baseline, raw DVI, routed DVI, GraphLite/PathScorer, embedding-guided KG-Trie, oracle-routing, and failure-analysis tables used in the report.
\end{itemize}
...
\subsection{Packaged Result Files}
\begin{itemize}
  \item Main experiment tables and ablations: \path{results_summary/EXPERIMENT_TABLES.md}.
  \item Compact metric summary: \path{results_summary/RESULTS.md}.
  \item Dataset download and loading links: \path{datasets/DATASET_LINKS.md}.
\end{itemize}
The raw generated prediction directories are intentionally not part of the submission package because they are large and regenerable. All file paths named in the artifact map and code-location list above are relative to the submitted package root.
