...
The GitHub artifact keeps the three method tracks separated. The main implementation is in \path{code/gcr-dvi}; the embedding-guided KG-Trie patch is in \path{code/embedding\_kgtrie}; and the standalone GraphLite prototype is in \path{code/graphlite}. The separation is deliberate: DVI, GraphLite, and embedding-guided path selection answer different engineering questions and should be testable independently before being merged into one pipeline.
...
\midrule
\path{code/gcr-dvi/workflow/predict\_dvi.py} & End-to-end DVI pipeline with KG-LLM and PathScorer verification modes. \\
\path{code/gcr-dvi/src/dvi/decomposer.py} & General-LLM JSON decomposer with fallback and cache support. \\
\path{code/gcr-dvi/src/dvi/intersector.py} & Tail-entity extraction, candidate-set construction, strict intersection, and relaxed diagnostics. \\
\path{code/gcr-dvi/src/dvi/answer\_aware.py} & Heuristic answer-type refinement and local graph expansion. \\
\path{code/gcr-dvi/src/dvi/path\_scorer.py} & Bi-encoder filtering and cross-encoder reranking for enumerated paths. \\
\path{code/embedding\_kgtrie/overlay/src/qa\_prompt\_builder.py} & Embedding-guided path ranking and KG-Trie construction overlay. \\
\path{code/graphlite/lite\_framework/entity\_level\_decoder.py} & Standalone GraphLite entity-level graph enumeration and scoring prototype. \\
\path{results\_summary/EXPERIMENT\_TABLES.md} & GitHub-readable experiment tables corresponding to the report. \\
\bottomrule
\end{tabular}
